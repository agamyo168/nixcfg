      ...
      settings =
        ...
        in
        {
          spawn-at-startup = [
            ...
            } # to fix wl clipboards disappearing
          ]
          ++ (map (cmd: {
            command = [
              "sh"
          ...
          window-rules = [
            ...
            }
            {
              matches = [
                {
                  app-id = "^notitg-";
                  title = "Whoops! NotITG Has Crashed!";
                }
              ];
              open-floating = true;
              min-width = 600;
          ...
          binds =
            ...
            in
            {
              ...
              "XF86Launch1".action =
                sh "${lib.getExe pkgs.rofi-rbw-wayland} -a copy -t password --clear-after 20";
            ...
            // (lib.attrsets.listToAttrs (
              builtins.concatMap (
                i: with config.hm.lib.niri.actions; [
                  {
                    name = "Mod+${toString i}";
              ...
              if config.modules.desktop.wob.enable then
                ...
                in
                {
                  "XF86AudioRaiseVolume".action =
                    sh "wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 10%+ && wpctl get-volume @DEFAULT_AUDIO_SINK@ | sed 's/[^0-9]//g' > ${wobSock}";
                  "XF86AudioRaiseVolume".allow-when-locked = true;
                  "XF86AudioLowerVolume".action =
                    sh "wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 10%- && wpctl get-volume @DEFAULT_AUDIO_SINK@ | sed 's/[^0-9]//g' > ${wobSock}";
                  "XF86AudioLowerVolume".allow-when-locked = true;
                  "XF86AudioMute".action =
                    sh "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle && (wpctl get-volume @DEFAULT_AUDIO_SINK@ | grep -q MUTED && echo 0 > ${wobSock}) || wpctl get-volume @DEFAULT_AUDIO_SINK@ | sed 's/[^0-9]//g' > ${wobSock}";
                  "XF86AudioMute".allow-when-locked = true;
                  "XF86MonBrightnessUp".action =
                    sh "${lib.getExe pkgs.brightnessctl} s +5% && ${lib.getExe pkgs.brightnessctl} -m | awk -F, '{ print $4 }' | sed 's/.$//' > ${wobSock}";
                ...
                (
                  if config.modules.desktop.mako.osd then
                    {
                      ...
                      "XF86MonBrightnessUp".allow-when-locked = true;
                      "XF86MonBrightnessDown".action =
                        sh "${lib.getExe config.modules.desktop.mako.backlightScript} down";
                      "XF86MonBrightnessDown".allow-when-locked = true;
                    }
                  else
                    {
